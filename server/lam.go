...
func genHandler() {
	if len(os.Args) >= 3 {
		...
		if !checkTagDuplicate(tag) {
			...
			cliLogger("Tag duplicated. Please run \"lam list\" to see stored tags")
		}
	} else {
		helpHandler()
	}
...
func delHandler() {
	if len(os.Args) >= 3 {
		...
		if checkTagDuplicate(tag) {
			...
			cliLogger("No such tag named \"" + tag + "\". Please run \"lam list\" to see stored tags")
		}
	} else {
		helpHandler()
	}
...
func firstHandler() {
	// Prompt
	fmt.Println("It's your first time using LAM, let's configure it first.")
	// Database config
	...
	saveConf()
	// Finish
	fmt.Println("All settled, enjoy using LAM!")
}
